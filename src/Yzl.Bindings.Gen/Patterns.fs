...
module Patterns =
    ...
    let (|Properties|_|) (s: JsonSchema) = s.Properties |> matchNonEmpty

    let (|PatternProperties|_|) (s: JsonSchema) = s.PatternProperties |> matchNonEmpty
    ...
    let (|String|_|) (s: JsonSchema) = s |> matchType JsonObjectType.String

    let (|Integer|_|) (s: JsonSchema) = s |> matchType JsonObjectType.Integer

    let (|Number|_|) (s: JsonSchema) = s |> matchType JsonObjectType.Number

    let (|Boolean|_|) (s: JsonSchema) = s |> matchType JsonObjectType.Boolean
    ...
    let (|Definitions|_|) (s: JsonSchema) = s.Definitions |> matchNonEmpty
