...
module Core =
    ...
    /// </summary>
    let inline lift (x: ^a) : ^b =
        ((^a or ^b): (static member ToYzl: ^a -> ^b) x)

    let inline liftString (x: ^a) : ^b =
        ((^a or ^b): (static member ToYzlString: ^a -> ^b) x)
    ...
    type Node =
        ...
        static member ToYzl(source: int list) : Node =
            SeqNode(source |> List.map (Int >> Scalar))

        static member ToYzl(source: double list) : Node =
            SeqNode(source |> List.map (Float >> Scalar))

        static member ToYzl(source: bool list) : Node =
            SeqNode(source |> List.map (Bool >> Scalar))
    ...
    and [<AbstractClass; Sealed>] Yzl =
        static member named(value: Node, [<CallerMemberName; Optional; DefaultParameterValue("")>] name: string) =
            Named(Name name, value)

        /// Creates a named string scalar node from F# string or Yzl.Str
        static member inline str< ^T when (^T or Str): (static member ToYzlString: ^T -> Str)>
            (value: ^T, [<CallerMemberName; Optional; DefaultParameterValue("")>] name: string)
            =
        ...
        static member map
            (value: NamedNode list, [<CallerMemberName; Optional; DefaultParameterValue("")>] name: string)
            =
            Yzl.named (MapNode value, name)

        /// Creates a named sequence node
        static member inline seq<'T when ('T or Node): (static member ToYzl: 'T -> Node)>
            (value: 'T list, [<CallerMemberName; Optional; DefaultParameterValue("")>] name: string)
            =
...
module Render =
    ...
    module Yzl =
        ...
        let renderYaml (opts: RenderOptions) (yaml: Node) =
            ...
            let rec render (indent: string) this parent =
                ...
                let normalizeIndent (s: string) =
                    if not opts.multiLineRelativeIndent then
                        s
                    else
                        let lines =
                            let fixFirst z =
                        ...
                        let existingIndent (z: string option) =
                            match z with
                            | None -> Empty
                            | Some z -> z.[0 .. z.IndexOf(z.TrimStart()) - 1]
                ...
                let stringScalar =
                    function
                    | Plain z -> $"%s{nullOr z}\n"
                    | SingleQuoted z -> $"'%s{nullOr z}'\n"
                    | DoubleQuoted z -> $"\"%s{nullOr z}\"\n"
                    | Folded z -> $">\n%s{nullOr z |> normalizeIndent}\n"
                    | FoldedStrip z -> $">-\n%s{nullOr z |> normalizeIndent}\n"
                    | FoldedKeep z -> $">+\n%s{nullOr z |> normalizeIndent}\n"
                    | Literal z -> $"|\n%s{nullOr z |> normalizeIndent}\n"
                    | LiteralStrip z -> $"|-\n%s{nullOr z |> normalizeIndent}\n"
                ...
                let renderMap ms =
                    let mapElem i m =
                        ...
                        match (t, c) with
                        | Empty, NoNode -> ()
                        | _ -> $"%s{mapIndent}%s{t}:%s{eolOrSpace c}" |> append
        ...
        /// </example>
        let inline render (obj: ^a) =
            renderYaml RenderOptions.Default (lift obj)
