...
module Core =
  ...
  let tests =
    testList "generate" [
      testCase "K8s deployment resource"
      <| fun _ ->
      ...
      test "Top-level sequence" {
        ...
        let single x =
          [ name x
            ...
            fieldref [ fieldpath ("metadata.annotations." + x.ToLower()) ] ]
...
"""
      ...
      test "Should render empty map as {}" {
        ...
        let actual =
          [ empty []
            "non-empty" .= [ "some-other" .= "value" ] ]

        "Rendering failed"
        |> Expect.equal (actual |> Yzl.render) "empty: {}\nnon-empty:\n  some-other: value\n"
      }
      ...
      test "Automatic builder" {
        ...
        "Rendering failed"
        |> Expect.equal (actual |> Yzl.render) "myMap:\n  otherMap:\n    aSeq:\n    - 99\n    - 2\n    - 3\n"
      }
